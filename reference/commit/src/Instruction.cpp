...
void StoreInstruction::output() const
{
    std::string dst_zhouyanlin = operands[0]->toStr();
    std::string src_zhouyanlin = operands[1]->toStr();
    std::string dst_type_zhouyanlin = operands[0]->getType()->toStr();
    std::string src_type_zhouyanlin = operands[1]->getType()->toStr();

    fprintf(yyout, "  store %s %s, %s %s, align 4\n", src_type_zhouyanlin.c_str(),
            src_zhouyanlin.c_str(), dst_type_zhouyanlin.c_str(), dst_zhouyanlin.c_str());
}
