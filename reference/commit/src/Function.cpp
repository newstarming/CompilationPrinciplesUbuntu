...
void Function::computeSdom()
{
    ...
    int *ancestors_zhouyanlin = new int[len_zhouyanlin];
    for (int i = 0; i < len_zhouyanlin; i++)
    {
        sdoms[i] = i;
        ancestors_zhouyanlin[i] = -1;
    }
    for (auto it = preOrder2DFS.rbegin(); (*it)->block != entry; it++)
    {
        ...
        {
            int z = eval((*it1)->order, ancestors_zhouyanlin);
            if (sdoms[z] < sdoms[s])
                sdoms[s] = sdoms[z];
        }
        ancestors_zhouyanlin[s] = (*it)->parent->num;
    }
    delete[] ancestors_zhouyanlin;
...
void Function::computeDomFrontier()
{
    for (auto block_zhouyanlin : block_list)
    {
        if (block_zhouyanlin->getNumOfPred() >= 2)
        {
            for (auto it = block_zhouyanlin->pred_begin(); it != block_zhouyanlin->pred_end(); it++)
            {
                int runner = (*it)->order;
