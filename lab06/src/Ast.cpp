...
void IfStmt::genCode() {  //这里的if是针对if的
    Function* func;
...
void CompoundStmt::genCode() {//这里的compound是针对复合语句的
    // Todo
    if (stmt)
        stmt->genCode();
}

void SeqNode::genCode() {//这里的seq是针对序列的
    // Todo
    stmt1->genCode();
    stmt2->genCode();
}
...
void UnaryExpr::genCode() {//这里的unary是针对一元表达式的
    // Todo
    expr->genCode();
    if (op == NOT) {
        BasicBlock* bb = builder->getInsertBB();
